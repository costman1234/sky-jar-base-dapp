...
const source = await readFile(resolve(process.cwd(), "contracts/SkyJar.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "SkyJar.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["SkyJar.sol"]?.SkyJar;
...
console.log(`Deploying SkyJar from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SKY_JAR_CONTRACT_ADDRESS to this contract address in Vercel.");
